...
import { APIPromise } from '@hyperspell/hyperspell/core/api-promise';

import util from 'node:util';
import Hyperspell from '@hyperspell/hyperspell';
import { APIUserAbortError } from '@hyperspell/hyperspell';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['HYPERSPELL_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Hyperspell) => {
  ...
  test('maxRetries option is correctly set', () => {
    ...
    // default
    const client2 = new Hyperspell({ apiKey: 'My API Key', userID: 'My User ID' });
