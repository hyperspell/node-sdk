...
import { APIUserAbortError } from 'hyperspell';
import { Headers } from 'hyperspell/core';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['HYPERSPELL_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Hyperspell({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['HYPERSPELL_BASE_URL'] = 'https://example.com/from_env';
      const client = new Hyperspell({ bearerToken: 'My Bearer Token' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Hyperspell({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Hyperspell({ bearerToken: 'My Bearer Token' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Hyperspell({ bearerToken: 'My Bearer Token', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Hyperspell({ bearerToken: 'My Bearer Token', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Hyperspell({ bearerToken: 'My Bearer Token', fetch: testFetch });
